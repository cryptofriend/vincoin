...
Copyright (c) 2017-2019, The Vincoin Cash Project  
...
### On Linux for Android (using docker):
        ...
        cd utils/build_scripts/ && docker build -f android32.Dockerfile -t vincoin-android .
        # Create container
        docker create -it --name vincoin-android vincoin-android bash
        # Get binaries
        docker cp vincoin-android:/opt/android/vincoin/build/release/bin .
...
To list all available options, run `./bin/vincoind --help`.  Options can be
...
To run in background:
    ...
    ./bin/vincoind --log-file vincoind.log --detach

To run as a systemd service, copy
[vincoind.service](utils/systemd/vincoind.service) to `/etc/systemd/system/` and
[vincoind.conf](utils/conf/vincoind.conf) to `/etc/`. The [example
service](utils/systemd/vincoind.service) assumes that the user `vincoin` exists
and its home is the data directory specified in the [example
config](utils/conf/vincoind.conf).

If you're on Mac, you may need to add the `--max-concurrency 1` option to
vincoin-wallet-cli, and possibly vincoind, if you get crashes refreshing.
...
`DNS_PUBLIC=tcp TORSOCKS_ALLOW_INBOUND=1 torsocks vincoind --p2p-bind-ip 127.0.0.1 --no-igd`

TAILS ships with a very restrictive set of firewall rules. Therefore, you need to add a rule to allow this connection too, in addition to telling torsocks to allow inbound connections. Full example:

`sudo iptables -I OUTPUT 2 -p tcp -d 127.0.0.1 -m tcp --dport 16062 -j ACCEPT`

`DNS_PUBLIC=tcp torsocks ./vincoind --p2p-bind-ip 127.0.0.1 --no-igd --rpc-bind-ip 127.0.0.1 --data-dir /home/amnesia/Persistent/your/directory/to/the/blockchain`

`./vincoin-wallet-cli`

## Using readline

While vincoind and vincoin-wallet-cli do not use readline directly, most of the functionality can be obtained by running them via rlwrap. This allows command recall, edit capabilities, etc. It does not give autocompletion without an extra completion file, however. To use rlwrap, simply prepend `rlwrap` to the command line, eg:

`rlwrap bin/vincoin-wallet-cli --wallet-file /path/to/wallet`
...
```
gdb /path/to/vincoind `pidof vincoind` 
```
...
When it terminates with an output along the lines of "Segmentation fault (core dumped)", there should be a core dump file in the same directory as vincoind.

You can now analyse this core dump with `gdb` as follows:

`gdb /path/to/vincoind /path/to/dumpfile`

Print the stack trace with `bt`

* To run vincoin within gdb:

Type `gdb /path/to/vincoind`

Pass command-line options with `--args` followed by the relevant arguments

Type `run` to run vincoind

## Analysing Memory Corruption

We use the tool `valgrind` for this.

Run with `valgrind /path/to/vincoind`. It will be slow.
...
`cd ~/vincoin/external/db_drivers/liblmdb && make`
